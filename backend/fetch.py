...
def fetch_location_data(
    ...
    if cached_min is None:
        print("    DB cache      : EMPTY — will fetch full range from API")
    ...
    if not chunked_ranges:
        print("   🚀 Source        : DB ONLY (no API call needed)")
    ...
    for from_dt, to_dt in chunked_ranges:
        ...
        if not api_df.empty:
            ...
            print("     API returned  : 0 rows (empty or error)")
    ...
    if not frames:
        print("   ❌ Final result  : no data available")
        print(f"{'=' * 60}\n")
        return pd.DataFrame()
...
def _fetch_from_api(
    ...
    try:
        ...
        if not isinstance(api_data, list) or len(api_data) == 0:
            print("        API response is empty or not a list")
            return pd.DataFrame()

        df = pd.DataFrame(api_data)
        if "time" in df.columns:
            df["timestamp"] = (
                pd.to_datetime(df["time"], unit="s", utc=True).dt.tz_convert("Europe/Bucharest").dt.tz_localize(None)
            )
