    ...
    if (latestParking >= latestCars) {
      ...
      return {
        ...
        data: [
          { name: "Utilized Parking", value: latestCars, fill: "#ef4444" },
          {
            ...
            value: latestParking - latestCars,
            fill: "#10b981",
          },
        ],
        cars: latestCars,
      ...
      return {
        ...
        data: [
          ...
          {
            name: "Parking Deficit",
            value: latestCars - latestParking,
            fill: "#f59e0b",
          },
        ],
        cars: latestCars,
      ...
      <div className="grid grid-cols-1 gap-4 px-4 py-4 bg-muted/50 border-b sm:grid-cols-3 sm:px-6">
        ...
        <div className="space-y-1 min-w-0">
          <div className="flex items-center gap-2">
            <StatusIcon
              className={`h-4 w-4 ${
                pieData.hasDeficit ? "text-orange-600" : "text-green-600"
              }`}
            />
            <p className="text-xs text-muted-foreground font-medium">
          ...
          >
            {pieData.hasDeficit ? "-" : "+"}
            {formatNumber(pieData.difference)}
          </p>
        </div>
      </div>
              ...
              {pieData.hasDeficit ? (
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Only{" "}
                    <span className="font-bold">
                      {pieData.utilizationRate.toFixed(1)}%
                    </span>{" "}
                    of vehicles can be accommodated with current parking
                    infrastructure.
                  </p>
                  <div className="mt-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                    <p className="text-xs text-orange-800 font-medium">
                      ⚠️ Urgent Action Needed: Consider expanding parking
                      infrastructure or promoting alternative transportation.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  <div className="mt-3 p-3 bg-green-50 rounded-lg border border-green-200">
                    <p className="text-xs text-green-800 font-medium">
                      ✅ Good Infrastructure: Sufficient parking available for
