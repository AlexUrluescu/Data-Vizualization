...
export default function ChartCorrelation({
  ...
  const mergedData = React.useMemo(() => {
    ...
    if (pollutionData && pollutionData.length > 0) {
      pollutionData.forEach((item) => {
        ...
        const metrics = Object.keys(item).filter(
          (key) => key !== "year" && key !== "_id" && key !== "cityId"
        );
        const avgPollution =
          metrics.length > 0
            ? metrics.reduce((sum, key) => sum + (item[key] as number), 0) /
              metrics.length
            : 0;
        ...
        dataMap.set(item.year, {
          ...existing,
          year: item.year,
          pollutionIndex: avgPollution,
        });
      });
    }

    // Calculate derived metrics
    const finalData: CorrelationDataPoint[] = Array.from(dataMap.values())
      .map((item) => {
  ...
  const correlations = React.useMemo(() => {
    ...
    return {
      ...
      populationGrowth:
        ((latest.population - first.population) / first.population) * 100,
      parkingGrowth: ((latest.parking - first.parking) / first.parking) * 100,
      pollutionChange:
        latest.pollutionIndex && first.pollutionIndex
          ? ((latest.pollutionIndex - first.pollutionIndex) /
              first.pollutionIndex) *
            100
          : null,
      avgCarsPerCapita:
        filteredData.reduce((sum, d) => sum + d.carsPerCapita, 0) /
        filteredData.length,
      avgParkingUtil:
        filteredData.reduce((sum, d) => sum + d.parkingRatio, 0) /
        filteredData.length,
    };
  }, [filteredData]);
      ...
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 bg-gradient-to-r from-purple-50 via-pink-50 to-orange-50 sm:flex-row">
        <div className="grid flex-1 gap-1">
          <CardTitle className="text-2xl">
            🔗 Urban Data Correlation Analysis
          </CardTitle>
          <CardDescription className="text-base">
            Comprehensive view of how cars, population, parking, and pollution
            interact over time
          </CardDescription>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 px-4 py-4 bg-gradient-to-r from-purple-50/50 to-orange-50/50 border-b sm:gap-4 sm:px-6 md:grid-cols-3 lg:grid-cols-6">
          <div className="space-y-1 min-w-0">
            ...
            <p
              className={`text-lg font-bold sm:text-xl ${
                correlations.carsGrowth > 0
                  ? "text-orange-600"
                  : "text-green-600"
              }`}
            >
              {correlations.carsGrowth >= 0 ? "+" : ""}
              {correlations.carsGrowth.toFixed(1)}%
            </p>
          </div>
          <div className="space-y-1 min-w-0">
            <p className="text-xs text-muted-foreground font-medium truncate">
            ...
            <p className="text-lg font-bold text-blue-600 sm:text-xl">
              {correlations.populationGrowth >= 0 ? "+" : ""}
              {correlations.populationGrowth.toFixed(1)}%
            </p>
          </div>
          <div className="space-y-1 min-w-0">
            <p className="text-xs text-muted-foreground font-medium truncate">
            ...
            <p
              className={`text-lg font-bold sm:text-xl ${
                correlations.parkingGrowth > 0
                  ? "text-green-600"
                  : "text-red-600"
              }`}
            >
              {correlations.parkingGrowth >= 0 ? "+" : ""}
              {correlations.parkingGrowth.toFixed(1)}%
            </p>
          </div>
          {correlations.pollutionChange !== null && (
            <div className="space-y-1 min-w-0">
              ...
              <p
                className={`text-lg font-bold sm:text-xl ${
                  correlations.pollutionChange > 0
                    ? "text-red-600"
                    : "text-green-600"
                }`}
              >
                {correlations.pollutionChange >= 0 ? "+" : ""}
                {correlations.pollutionChange.toFixed(1)}%
              </p>
            </div>
          )}
          <div className="space-y-1 min-w-0">
          ...
          <div className="space-y-1 min-w-0">
            ...
            <p
              className={`text-lg font-bold sm:text-xl ${
                correlations.avgParkingUtil > 100
                  ? "text-red-600"
                  : "text-green-600"
              }`}
            >
            ...
            {correlations && (
              <>
                <li>
                  ...
                  {correlations.carsGrowth > correlations.populationGrowth
                    ? "growing faster"
                    : "growing slower"}{" "}
                  than population growth
                </li>
                <li>
                  • Average {correlations.avgCarsPerCapita.toFixed(0)} cars per
                  1,000 residents in this period
                </li>
                <li>
                  • Parking infrastructure is{" "}
                  {correlations.parkingGrowth > correlations.carsGrowth
                    ? "keeping pace with"
                    : "falling behind"}{" "}
                  vehicle growth
                </li>
                {correlations.pollutionChange !== null && (
                  <li>
                    ...
                    {correlations.pollutionChange > 0
                      ? "increased"
                      : "decreased"}{" "}
                    by {Math.abs(correlations.pollutionChange).toFixed(1)}%
                  </li>
                )}
              </>
            )}
