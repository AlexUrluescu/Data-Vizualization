...
export default function ChartPollutionTrends({
  ...
  const filteredData = React.useMemo(() => {
    ...
    return pollutionData.filter(
      (item) => item.year >= currentYear - yearsToShow
    );
  ...
  const averages = React.useMemo(() => {
    ...
    availableMetrics.forEach((metric) => {
      sums[metric] =
        filteredData.reduce((acc, item) => acc + (item[metric] || 0), 0) /
        filteredData.length;
    });
    ...
    <Card className="pt-0 transition-all duration-300 hover:shadow-lg">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          ...
          <CardDescription>Air pollution metrics over time</CardDescription>
        </div>
