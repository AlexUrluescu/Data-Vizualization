...
import { ComposedChart, Line, Area, CartesianGrid, XAxis, YAxis } from "recharts";
...
export default function ChartComparison({
  ...
  const mergedData = React.useMemo(() => {
    const dataMap = new Map<number, { year: number; cars?: number; population?: number }>();
    ...
    populationData.forEach((item) => {
      ...
      if (existing) {
        ...
        dataMap.set(item.year, { year: item.year, population: item.population });
      }
    });
  ...
  const carsPerCapitaData = React.useMemo(() => {
    return filteredData.map((item) => {
      const ratio = item.cars && item.population
  ...
  const avgRatio = React.useMemo(() => {
    ...
    return validRatios.reduce((sum, d) => sum + d.ratio, 0) / validRatios.length;
  }, [carsPerCapitaData]);
  ...
  const ratioChange = React.useMemo(() => {
    ...
    return { change, changePercent, trend: changePercent > 0 ? "up" : changePercent < 0 ? "down" : "stable" };
  }, [carsPerCapitaData]);
            ...
            <div className="space-y-2 min-w-0">
              ...
              <div className="flex items-baseline gap-2">
                <p className={`text-3xl font-extrabold sm:text-4xl ${
                  ratioChange.trend === "up" ? "text-green-600" :
                  ratioChange.trend === "down" ? "text-red-600" : "text-gray-600"
                }`}>
                  {ratioChange.changePercent >= 0 ? "+" : ""}{ratioChange.changePercent.toFixed(1)}%
                </p>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed break-words">
                From {carsPerCapitaData[0].ratio.toFixed(0)} to {carsPerCapitaData[carsPerCapitaData.length - 1].ratio.toFixed(0)} cars per 1,000 people
              </p>
            </div>
          )}
        </div>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        ...
        >
          <ComposedChart data={filteredData}>
            <defs>
              <linearGradient id="gradient-cars-comp" x1="0" y1="0" x2="0" y2="1">
