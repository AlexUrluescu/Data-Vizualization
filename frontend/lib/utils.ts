...
export function calculateChartMetrics(
  ...
  const firstValue = Object.values(sortedData[0]).find(
    (v) => typeof v === "number" && v !== sortedData[0].year
  ) || 0;
  const lastValue = Object.values(sortedData[sortedData.length - 1]).find(
    (v) => typeof v === "number" && v !== sortedData[sortedData.length - 1].year
  ) || 0;
